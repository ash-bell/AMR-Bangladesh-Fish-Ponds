...
# script from bbtools pipeline

mkdir REPLACE_ME && cd REPLACE_ME

#Interleave reads
reformat.sh in1=$READS_DIR/REPLACE_ME_r1.fq.gz in2=$READS_DIR/REPLACE_ME_r2.fq.gz out=interleave.fq.gz -Xmx1000g
...
#Human
bbmap.sh build=1 path=$WKDIR minid=0.95 maxindel=3 bwr=0.16 bw=12 quickmatch fast minhits=2 qtrim=rl trimq=10 untrim in=temp.fq.gz outu=ex_human.fq.gz threads=8 -Xmx1000g
